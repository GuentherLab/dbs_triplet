function P09_detect_artifact_criteria_C(SUBJECT, param)

%% Detect artifacts
% creates an artifact annotation table

% CRITERIA C paramaters valus
CRITERIA = 'C'; %identifier for the criteria implemented in this script
...
PATH_PROTOCOL = 'Z:\DBS\Batch\P09_artifact_criteria_C';
...
if ~exist('el_band','var')
  param = readtable('artifact_C_params.txt');
  param = param(param.subject == "default",:);
end
...
%   ft_databrowser(cfg,D_hpf_ecog_env);
  ...
  D_hpf_eltype_env_log10 = bml_apply(@(x) log10(x),D_hpf_eltype_env);
