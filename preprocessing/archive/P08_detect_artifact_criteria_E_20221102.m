...
PATH_PROTOCOL = 'Z:\DBS\Batch\P09_artifact_criteria_E';
...
session_ids = unique(trials.session_id);
...
for itrial = 1:ntrials-1
    ...
    if trials.session_id(itrial) == trials.session_id(itrial+1) && ... % if trials are in the same session
...
for ises = 1:nses
    trialmatch = find(trials.session_id == session_ids(ises)); % row indices
