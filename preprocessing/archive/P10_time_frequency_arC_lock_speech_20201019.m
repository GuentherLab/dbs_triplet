% % % % % % % % function P10_time_frequency_arC_lock_speech
...
trial_events = bml_annot_read_tsv([PATH_ANNOT filesep 'sub-' SUBJECT '_ses-' SESSION '_task-' TASK '_annot-trials.tsv']); 


%% defining epochs and baseline for time frequency analysis
prod_onset = trial_events.audio_go_offset;
prod_offset = trial_events.keypress_time;
...
event_annot.ends_error = sqrt(event_annot.ends_rstd.^2 + 0.01.^2);
  ...
  freqs=[60 120 180 240]; % notch filters for 60hz harmonics
  cfg=[];
  cfg.hpfilter='yes';
  cfg.hpfreq=1;
  cfg.hpfilttype='but';
