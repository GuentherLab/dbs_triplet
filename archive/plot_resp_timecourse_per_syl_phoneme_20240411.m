...
if isempty(trial_grouping_var) % plot all trials in a single trace
    triptabvals = true(ntrials,1); 
    full_var_string = 'all_trials';
elseif ~isempty(trial_grouping_var)
...
% error bars
if show_error_bars
    ebars_to_plot = [resp_align.sem_lims(1,:), fliplr(resp_align.sem_lims(2,:))];
    hfill = fill([xtime, fliplr(xtime)], ebars_to_plot, [0.8 0.8 0.8],'HandleVisibility','off'); % standard error
        hfill.LineStyle = 'none'; % no border
    hold on 
end

timecourses_to_plot = cell2mat(resp_grpd.resp_mean(groupval_inds_to_plot,:))';
