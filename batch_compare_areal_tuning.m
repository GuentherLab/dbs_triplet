...
%  clear
%  load('Z:\DBS\Analysis\triplet_results_am\esp_all-subjects_hg_ar-E_ref-CTAR_denoised.mat')

 [paramlist] = expandarray('params_to_plot_20231217.csv'); % file stored in dbs_triplet code folder

%%%% need to edit cod to use this format of pamramlist - each param is not split into 3 syllable positions
% paramlist = table2cell(readtable('params_to_plot_allpos_constit.tsv','ReadVariableNames', false,'Delimiter','tab','FileType','text'));
...
pt = paramlist';  % select from transposed params so that it matches subplot ordering
