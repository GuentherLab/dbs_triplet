...
%% 
%PART 1 DT
% Define CVC combinations array
vc_combinations = ["vahtoosee", "veetahsoo", "sooveetah", "tooveeghah", "gheeghahvah", "soovahghee", "seetahvoo", "seegheetoo", "tahsooghee", "tahseeghoo"];

transitions1 = cell(1, numel(vc_combinations));
transitions2 = cell(1, numel(vc_combinations));

% Loop through each CVC combination
for i = 1:numel(vc_combinations)
    vc = convertStringsToChars(vc_combinations(i));  % Convert to character array
    s1 = vc(1:3);  % First syllable
    s2 = vc(4:6);  % Second syllable
    s3 = vc(7:end);  % Third syllable

    % Extract transitions, adjusting for 'gh'
    transitions1{i} = strcat(extractAfter(s1, numel(s1) - 2), extractBefore(s2, 2));
    transitions2{i} = strcat(extractAfter(s2, numel(s2) - 2), extractBefore(s3, 2));
end

% Filter and combine unique transitions
CombinedTransitions = union(unique(transitions1), unique(transitions2));

% Remove specific elements ('egha' and 'ghee')
FilteredTransitions = CombinedTransitions(~ismember(CombinedTransitions, {'egha', 'ghee'}));

% Display results
disp('Filtered Transitions:');
disp(FilteredTransitions);
%%
%PART 2 DT
