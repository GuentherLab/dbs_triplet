...
function plot_phonotactic_vs_rt_logscale_jitter(reactiontime_table, phonotacticProbabilities, transitions, dbsID, trans_field)
    ...
    % Add jitter to phonotactic probabilities to avoid stacking
    jitter_strength = 0.05;  % Adjust this value to control the amount of jitter
    jitter = jitter_strength * randn(size(sorted_pp_values));
    jittered_pp_values = sorted_pp_values + jitter;
    
    % Create the plot
    figure;
    hold on;
    
    % Replace zero phonotactic probabilities with a small positive number (eps) for log scale
    jittered_pp_values(jittered_pp_values == 0) = eps;
    ...
    for i = 1:length(sorted_transitions)
        ...
        % Plot with error bars at the jittered phonotactic probability (x-axis) and mean RT (y-axis)
        errorbar(jittered_pp_values(i), mean_rt(sort_idx(i)), std_rt(sort_idx(i)), markers{mod(i-1, length(markers)) + 1}, ...
    ...
    xlim([min(jittered_pp_values)*0.8, max(jittered_pp_values)*1.2]);  % Add buffer
    ...
    saveas(gcf, sprintf('Phonotactic_Probability_vs_RT_LogScale_Jitter_%s_%s.png', dbsID, trans_field));
end
