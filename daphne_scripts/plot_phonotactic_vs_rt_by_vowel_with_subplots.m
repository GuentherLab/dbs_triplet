...
function plot_phonotactic_vs_rt_by_vowel_with_subplots(reactiontime_table, phonotacticProbabilities, transitions, dbsID, trans_field)
    % Group transitions by the first vowel
    vowel_groups = struct('a', {}, 'o', {}, 'e', {});
    ...
    vowels = fieldnames(vowel_groups);
    ...
    for v = 1:length(vowels)
        ...
        for i = 1:length(transitions_for_vowel)
            ...
            if numel(rt_values) > 1
                ...
                pp_values(i) = phonotacticProbabilities.(transition);
            end
        end
