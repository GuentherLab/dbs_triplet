...
for i_sub = 1:nsubs_to_analyze
    ...
    if exist(phonemeFilePath, 'file')
        ...
        subtable.phoneme_tables{i_sub}.seq1 = cell(height(subtable.phoneme_tables{i_sub}), 1);
        subtable.phoneme_tables{i_sub}.seq2 = cell(height(subtable.phoneme_tables{i_sub}), 1);
        subtable.phoneme_tables{i_sub}.seq3 = cell(height(subtable.phoneme_tables{i_sub}), 1);
        ...
        for trial = 1:length(unique_trials)
            ...
            if height(trial_phonemes) >= 6
                ...
                % Display sequences and reaction times for debugging
                disp(['Trial ID: ', num2str(trial_id), ', seq1: ', seq1, ', seq2: ', seq2, ', seq3: ', seq3]);
                disp(['seq1_RT: ', num2str(seq1_RT), ', seq2_RT: ', num2str(seq2_RT), ', seq3_RT: ', num2str(seq3_RT)]);
                ...
                % Display phonotactic probabilities for debugging
                disp(['pp1: ', num2str(pp1), ', pp2: ', num2str(pp2), ', pp3: ', num2str(pp3)]);

                % Store sequences, reaction times, and phonotactic probabilities
                row_indices = find(subtable.phoneme_tables{i_sub}.trial_id == trial_id);
                subtable.phoneme_tables{i_sub}.seq1(row_indices(1)) = {seq1};
                subtable.phoneme_tables{i_sub}.seq2(row_indices(1)) = {seq2};
                subtable.phoneme_tables{i_sub}.seq3(row_indices(1)) = {seq3};
