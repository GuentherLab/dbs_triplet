...
correlationResults_trans_orig1 = [];
correlationResults_trans_orig2 = [];
pValues_trans_orig1 = [];
pValues_trans_orig2 = [];
validDBSIDs = {};
...
for i = 1:length(dbsIDs)
    ...
    if istable(tripletTable) && ismember('error_trans_orig1', tripletTable.Properties.VariableNames) && ...
       ismember('phonotactic_prob_trans_1', tripletTable.Properties.VariableNames)
        ...
        % Store the results
        correlationResults_trans_orig1 = [correlationResults_trans_orig1; r_trans1];
        pValues_trans_orig1 = [pValues_trans_orig1; p_trans1];
        correlationResults_trans_orig2 = [correlationResults_trans_orig2; r_trans2];
        pValues_trans_orig2 = [pValues_trans_orig2; p_trans2];
...
for i = 1:length(validDBSIDs)
    fprintf(fileID, '%s\t%.2f\t%.4f\t%.2f\t%.4f\n', validDBSIDs{i}, correlationResults_trans_orig1(i), pValues_trans_orig1(i), correlationResults_trans_orig2(i), pValues_trans_orig2(i));
end
fclose(fileID);
