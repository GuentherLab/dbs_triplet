% Function to create a scatter plot for phonotactic probability vs. mean reaction time
function plot_phonotactic_vs_rt_scatter(reactiontime_table, phonotacticProbabilities, transitions, dbsID, trans_field)
    ...
    pp_values = zeros(size(transitions));  % Array to store phonotactic probabilities
    ...
    for i = 1:length(transitions)
        ...
        if numel(rt_values) > 1  % Ensure there are enough data points
            ...
            pp_values(i) = phonotacticProbabilities.(transition);  % Store phonotactic probability
